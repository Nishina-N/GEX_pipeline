...
**解釈**: プット主体（−NETGEX）のストライクで、MMは**ショートガンマ**になりやすい帯。ヘッジが順張り（下落時にさらに売る）に働くため、**Call Wallのような「下値を支える床」ではない**。むしろ**下に割れると下落が加速しやすい閾値（トラップドア）**として読む（§7参照）。目印・ピン（磁石）水準としての意味はあるが、方向性のある「支持帯」と混同しない。

> ⚠️ **Call Wallとの非対称性**: Call Wall（＋NETGEX）はMMがロングガンマ＝逆張りヘッジで**減衰的（本物の抵抗/ピン）**。Put Wall（−NETGEX）はMMがショートガンマ＝順張りヘッジで**増幅的**。両者を対称な「壁」として扱わないこと。なお「支持か加速か」は実際に誰がそのプットを保有するか（ディーラーのロング/ショート）で決まり、仮定フローのGEXでは原理的に判別できない（約定符号判定が必要）。
...
### STEP 2.5: Transition Zone の逸脱（上抜け／下抜け）と帯幅

STEP 2 は「Spot が TZ の内側にある」ことを前提とした位置判定です。**Spot が TZ の外に出ている場合は STEP 2 を適用せず、以下で読みます**。
`transition_zone = {'lower': putWall, 'upper': callWall}` なので、判定は `Spot` と `lower`/`upper` の比較のみです。

| 状態 | 判定式 | 解釈と注記 |
|------|--------|-----------|
| **TZ上抜け** | `Spot > upper (= CW)` | 主抵抗をすでに超えている。次のCW2までは空白地帯で**上方に加速しやすい**（§7）。ただし ＋γ かつ |totalGEX| が厚い場合は、上のストライクに壁が再構築されて**ピン留め（もみ合い）に転じる**ことも多い。記事では「CWを上抜け済み。次の抵抗はCW2の◯◯」と明記する |
| **TZ下抜け** | `Spot < lower (= PW)` | 下値のフシを割っている。**−γ ならヘッジがトレンドを加速させるため下方加速の警戒最大**。＋γ のうちに割れた場合は、HVL割れ＝−γ転落までの距離を必ず併記する |
| **TZ内** | `lower ≤ Spot ≤ upper` | 通常。STEP 2 の中点判定に従う |
| **壁が逆転／至近クラスタ** | `upper < lower`（CW < PW） | データ異常ではなく、**CWとPWが Spot のごく近くに密集している**状態。TZの内外判定は保留し、「上下の壁が至近に集中しており、どちらに抜けても値が飛びやすい」と記述する |

**帯幅（クッション）の注記**: `帯幅% = (upper − lower) / Spot × 100`

| 帯幅% | 注記 |
|-------|------|
| < 1% | **クッションが極めて薄い**。イベント一つで上下どちらの壁も抜けうる → 値幅が出る前提で書く |
| 1〜3% | 通常のレンジ幅 |
| > 3% | 壁が遠く、TZ内での自由度が高い。当面はWall到達を待つ展開 |

> ⚠️ **逸脱は「昨日どうだったか」とセットで書く**：同じ「TZ上抜け」でも、前日からの上抜けなのか、CWが切り下がった結果として相対的に外に出たのかで意味が正反対になる。
> **価格が動いて抜けた**のか **壁が動いて外に出た**のかを必ず区別する（HVLの「機械的」レジーム反転と同じ注意点）。
