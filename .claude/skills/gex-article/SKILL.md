...
```yaml
---
title: GEXからみた相場観と注目株 {date}
tags: [GEX, ガンマエクスポージャー, オプション, 米国株, 相場分析]
date: {date}
status: draft
---
```
...
## Transition Zone の逸脱注記（TZ上抜け／下抜け）

`transition_zone = {lower: PW, upper: CW}`。**Spot が TZ の外に出ている銘柄は、詳細セクションで必ず一言注記する**（詳細はガイドライン §6 STEP 2.5）。

- **上抜け（Spot > CW）**: 「CW ◯◯ を上抜け済み。次の抵抗は CW2 の ◯◯」と書く。＋γかつ厚い場合は「上のストライクに壁が組み直されればピン留めに転じうる」も添える。
- **下抜け（Spot < PW）**: 「PW ◯◯ を割れており、下方向に加速しやすい状態」。−γならその旨を重ねて強調、＋γならHVLまでの距離（−γ転落までの余地）を併記する。
- **壁の逆転（CW < PW）**: データ異常として無視せず、「上下の壁が至近に集中しており、どちらに抜けても値が飛びやすい」と記述する。
- **帯幅の注記**: `(CW − PW) / Spot` が **1%未満なら「クッションが薄い」と明記**する。指数（特にSPY/QQQ）では毎回チェックする。
- **必ず原因を区別する**: 「価格が動いて抜けた」のか「壁が動いて外に出た」のか。後者は前日値と並べて機械的な変化である旨を書く（HVLの機械的レジーム反転と同じ作法）。

## Call Wall / Put Wall の非対称性（文言の作法）

Call Wall と Put Wall は機構が非対称なので、同じ「壁」として扱わない。

- **Call Wall（＋NETGEX）**: ディーラーがロングガンマ → ヘッジは逆張り（減衰的）。**本物の上値抵抗／ピン**として「上値抵抗」と書いてよい。
- **Put Wall（−NETGEX）**: ディーラーがショートガンマ → ヘッジは順張り（増幅的）。**下値を支える「床」ではない**。下に割れるとむしろ下落が加速する閾値（ガイドライン §7）。
  - 「下値支持」「下値の支持帯」という**Call Wallと対称な表現は使わない**。
  - 代わりに **「下値のフシ／注視ライン」「割れると下方向に加速しやすい水準」** と書く。SQ接近時のピン（磁石）効果に触れるのは可だが、方向性のある支持と混同しない。
- 厳密には「支持か加速か」は誰がそのプットを保有するか（ディーラーのロング/ショート）で決まり、仮定フローのGEXでは判別できない。断定を避け、上記の中立表現に留める。

## 用語説明（各GEX一覧の直後に毎回そのまま挿入）

> Spot: 現在価格　／　GEX: ガンマエクスポージャー合計　／　HVL: 高ボラティリティレベル（GEXゼロクロス点）　／　CW: コールウォール（上値抵抗）　／　PW: プットウォール（下値のフシ。割れると下方向に加速しやすい閾値）　／　＋γ: ポジティブガンマ（安定圏）　／　−γ: ネガティブガンマ（不安定圏）

## 文体・ルール

- 対象読者: オプションの基本知識があるトレーダー。基礎説明は最小限。
- 簡潔・客観的。推奨トレードや投資助言は含めない。
- **トーン**: 硬くなりすぎない。**中性的で読みやすい丁寧語**を基調に、要所（Topics・サマリー・各セクションのつなぎ・まとめ）で「〜ですね」「〜していきましょう」程度の**軽い柔らかさ**を交える。ただし**入れすぎない**（1セクションに1つ程度の目安）。データの数値・レベル名・GEX解釈の正確さは保つ（柔らかくしても事実は崩さない）。箇条書きの定義部分は従来どおりカッチリで可。
